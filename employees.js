...
// ── GET /api/employees — list all (HR/Admin only) ────────
...
router.put('/:id', authorize('hr', 'admin', 'hr_manager'), async (req, res) => {
  try {
    const { data, error } = await supabase
      .from('employees').update(req.body).eq('id', req.params.id).select().single();
    if (error) throw error;

    await supabase.from('activity_logs').insert({
      user_id: req.user.id, action: 'UPDATE_EMPLOYEE',
      description: `Updated employee ID: ${req.params.id}`
    });
