...
router.get('/', authorize('admin', 'hr', 'hr_manager'), async (req, res) => {
  try {
    const { data, error } = await supabase
      .from('users')
      .select('id, email, full_name, roles, is_active, created_at, employee_id, employee:employees(first_name,last_name,employee_id,position,department:departments(name))')
      .order('full_name');
...
router.post('/', authorize('admin', 'hr_manager'), async (req, res) => {
  ...
  try {
    ...
    await supabase.from('activity_logs').insert({
      ...
      description: `Created user account: ${email}`
    });
...
// ── PUT /api/users/:id — update roles/status ─────────────
