...
// ── GET /api/payroll — list payroll runs (HR only) ───────
...
// ── GET /api/payroll/my-payslips — employee's own slips ──
...
// ── POST /api/payroll/compute — preview before saving ────
router.post('/compute', authorize('hr', 'admin', 'hr_manager', 'payroll_officer'), async (req, res) => {
  const {
    employee_id, basic_pay, allowances, overtime_pay,
    days_absent, lates_minutes, working_days
  } = req.body;

  if (!employee_id || !basic_pay)
    return res.status(400).json({ success: false, message: 'employee_id and basic_pay required' });
...
router.post('/run', authorize('hr', 'admin', 'hr_manager', 'payroll_officer'), async (req, res) => {
  ...
  try {
    ...
    const records = employees.map(emp => {
      ...
      const overtimePay = (att.overtime_minutes || 0) / 60 * hourlyRate * OT_RATE;
    ...
    const { error: recErr } = await supabase.from('payroll_records').insert(records);
    ...
    await supabase.from('activity_logs').insert({
      user_id: req.user.id, action: 'PROCESS_PAYROLL',
      description: `Payroll run: ${period_start} to ${period_end}, ${employees.length} employees`
    });
...
// ── GET /api/payroll/:run_id/records — all records for a run ──
